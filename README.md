<!--
**whimsical-tech/whimsical-tech** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

# Cristina Santana

🌇 Senior Frontend Engineer | React, Next.js, TypeScript | Berlin → Tokyo

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/acsfrontend)
[![Portfolio](https://img.shields.io/badge/Portfolio-8B5CF6?style=for-the-badge&logo=vercel)](https://whimsical-tech.vercel.app)
[![Qiita](https://img.shields.io/badge/Qiita-55C500?style=for-the-badge)](https://qiita.com/frontendpro)

日本語版をご覧になるには、スクロールしてください。

## 🔭 What I Build
- Real-time data dashboards & visualization tools, including vehicles' tracking with Google Maps SDK
- Checkout pages and integration with payment APIs
- Multi-language applications 🌏
- Responsive platforms from phones to Smart TVs
- Component systems & design libraries for teams

## 🌱 What I'm Learning
My priority is to improve my technical Japanese, so I'm reading Qiita articles everyday.

## 🛠️ Tech Stack
**Frontend:** React ⚛️ • Next.js • TypeScript • Vue.js • Javascript • HTML5 • CSS3


**Backend:** PHP • Magento • Next.js • Postgresql • Supabase

**Testing:** Jest • React Testing Library • Selenium • Cypress   

**DevOps:** Vercel • AWS • Docker  

<!-- Waiting to have at least 2 articles before unhiding this
## ✍️ What I'm Writing
I love writing not only code, but also documentation and articles!
Please check my portfolio's blog and my page on Qiita.
-->
## 📍 Currently
Living in Nakano, Tokyo • Looking for a full-time role to transition from student visa

## 💡 Why i18n Matters
As someone who once translated games word-by-word with a paper dictionary, I know how language barriers exclude people from content they love. Building accessible, internationalized products isn't just technical work — it's bridge-building. 🌉

## 🎯 Quick Stats
| Metric | Value |
|--------|-------|
| Experience | 12+ years |
| Performance Wins | 40% workflow boost • 30% load time reduction • #1 on Google Search |
| Languages | Portuguese (native) • English (fluent) • Japanese (N3) • German (intermediate) |

# クリスティーナ・サンタナ

🌇 シニアフロントエンドエンジニア | React、Next.js、TypeScript | ベルリン→東京

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/acsfrontend)
[![ポートフォリオ](https://img.shields.io/badge/ポートフォリオ-8B5CF6?style=for-the-badge&logo=vercel)](https://whimsical-tech.vercel.app)
[![Qiita](https://img.shields.io/badge/Qiita-55C500?style=for-the-badge)](https://qiita.com/frontendpro)

## 🔭 作るもの
- Google Maps SDK を使った車両追跡を含むリアルタイムデータダッシュボードと可視化ツール
- チェックアウトページと決済 API の統合
- 多言語対応アプリケーション 🌏
- スマートフォンからスマートテレビまで対応するレスポンシブプラットフォーム
- チーム向けコンポーネントシステムとデザインライブラリ

## 🌱 今学んでいること
技術的な日本語力を高めることを優先しているので、毎日Qiitaの記事を読んでいます。

## 🛠️ テックスタック
**フロントエンド:** React ⚛️ • Next.js • TypeScript • Vue.js • JavaScript • HTML5 • CSS3  
**バックエンド:** PHP • Magento • Next.js • PostgreSQL • Supabase  
**テスト:** Jest • React Testing Library • Selenium • Cypress  
**DevOps:** Vercel • AWS • Docker  

<!-- Waiting to have at least 2 articles before unhiding this
## ✍️ 執筆活動
コードだけでなく、ドキュメントや記事を書くことも大好きです！  
ポートフォリオのブログと Qiita のページをご覧ください。
-->

## 📍 現在
東京都中野区在住 • 学生ビザから就労ビザへの切替えのため正社員求職中 🇯🇵

## 💡 なぜ国際化が重要なのか
かつて紙の辞書でゲームを単語ごとの翻訳を楽しんでいた経験から、言語の壁がいかに人々を好きなコンテンツから遠ざけるかを知っています。アクセシブルで国際化された製品を作るのは技術作業だけではありません — それは架け橋を築くことです。🌉

## 🎯 クイック統計
| 項目 | 内容 |
|------|------|
| 経験年数 | 12 年以上 |
| パフォーマンス達成 | 40% ワークフロー改善 • 30% ローディング時間削減 • Google 検索第 1 位 |
| 言語 | ポルトガル語（母語）• 英語（堪能）• 日本語（N3）• ドイツ語（中級） |
